...
namespace Lamtest
{
    public class lambdaSerial : lambdaUtils
    {
        ...
        public lambdaSerial(Mainform mf)//mainform object passed to lambddaSerial object
        {
            ...
            this.mf = mf;//set reference of comport child to serial parent
...
// Read && write methods here_______________________________________________________
       ...
       {
           byte input=0;
           txtComOutput.AppendText("Read:" + Environment.NewLine + "Sts.| Dec. | Hex. " + Environment.NewLine);
           do
           {
               Thread.Sleep(150);//This is the minimum delay for my PC.  Longer delays might cause issues??? Perhaps not?
               input = readByte();
               System.Windows.Forms.Application.DoEvents();
           } while (input != 0 && input != 13);
           if (input != 13  && input != 0) { input = readByte(); }//Read CR
           /*
           lambdaUSB.ClearBuffer();
           lambdaRS232.DiscardInBuffer();
           clearBuffer();*/
